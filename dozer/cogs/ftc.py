...
class FTCInfo(Cog):
    ...
    async def ftc(self, ctx: DozerContext, *, team_name: str):
        ...
        """
        await self.searchteam.callback(ctx, team_name)
    ...
    async def searchteam(self, ctx: DozerContext, team_name: str):
        ...
        if team_name.isdigit():
            await self.team.callback(ctx, int(team_name))
            return
        if len(team_name) < 3:
        ...
        async with res:
            ...
            team_data = await res.json(content_type=None)
            if len(team_data) == 1:
                await self.team.callback(ctx, team_data[0]['number'])
                return
            manyteams = False
    ...
    async def topr(self, ctx: DozerContext, team_num: int):
        ...
        await self.opr.callback(ctx, team_num)
